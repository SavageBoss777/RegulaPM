    ...
    <div className="max-w-4xl mx-auto px-8 py-10">
      ...
      {briefs.length === 0 ? (
        <div className="text-center py-24">
          ...
          <p className="text-gray-500 text-sm mb-6 max-w-sm mx-auto">Create your first brief or seed demo data to get started.</p>
          <div className="flex items-center justify-center gap-3">
            <button onClick={() => router.push('/dashboard/new')} className="pill-button bg-[#3B4F6B] text-white text-sm hover:bg-[#2d3d52] flex items-center gap-2">
              <Plus className="w-4 h-4" /> Create Brief
            </button>
            <button onClick={handleSeed} disabled={seeding} className="pill-button border border-gray-200 text-gray-700 text-sm hover:bg-gray-50 flex items-center gap-2">
              {seeding ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
              Seed Demos
            </button>
          </div>
        ...
        <div className="grid gap-4">
          {briefs.map(b => (
            <button key={b.id} onClick={() => router.push(`/dashboard/briefs/${b.id}`)} className="bg-white rounded-2xl border border-gray-200 p-5 text-left hover:border-gray-300 hover:shadow-sm transition-all group">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h3 className="font-semibold text-[#111827]">{b.title}</h3>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColors[b.status] || 'bg-gray-100 text-gray-500'}`}>{b.status}</span>
                  </div>
                ...
                <ArrowRight className="w-4 h-4 text-gray-400 group-hover:text-[#3B4F6B] transition-colors" />
              </div>
            </button>
          ))}
        </div>
